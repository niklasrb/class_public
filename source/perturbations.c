...
int perturb_print_variables(double tau,
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (pba->has_dr == _TRUE_) {
        delta_dr += (-4.*a*H+a*dcdmdr_model_Q(pba, a, pvecback[pba->index_bg_H])/pvecback[pba->index_bg_rho_dr])*alpha;
      ...
      if (pba->has_dcdm == _TRUE_) {
        delta_dcdm += alpha*(-a*dcdmdr_model_Q(pba, a, pvecback[pba->index_bg_H])/pvecback[pba->index_bg_rho_dcdm]-3.*a*H);
  ...
  if (_scalars_) {
    ...
    if (pba->has_dcdm == _TRUE_) {
      ...
      dy[pv->index_pt_delta_dcdm] = -(y[pv->index_pt_theta_dcdm]+metric_continuity)
        - a * dcdmdr_model_Q(pba, a, pvecback[pba->index_bg_H])/pvecback[pba->index_bg_rho_dcdm] / k2 * metric_euler; /* dcdm density */
